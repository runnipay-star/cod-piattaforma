...
import { User, Affiliate, Manager, Transaction, UserRole, CustomerCareUser, Sale } from '../types';
...
interface PaymentsPageProps {
    ...
    sales: Sale[]; // Added sales prop to filter bonuses
...
const PaymentsPage: React.FC<PaymentsPageProps> = ({ user, fullUserObject, allUsersWithBalance, transactions, sales, onPayoutRequest, onTransferFunds, onAdminTransferFunds, onApproveTransaction, onRejectTransaction, onAddBonus }) => {
    ...
    const [activeTab, setActiveTab] = useState<'payout' | 'transfer' | 'history' | 'requests' | 'all_history' | 'admin_transfer' | 'add_bonus' | 'bonus_history'>(getDefaultTab());
    ...
    const availableToRequest = (fullUserObject.currentBalance || 0) - pendingPayoutsAmount;

    const bonusWorthyUsers = useMemo(() => {
        return allUsersWithBalance.filter(u => u.role === UserRole.AFFILIATE || u.role === UserRole.MANAGER || u.role === UserRole.CUSTOMER_CARE);
    }, [allUsersWithBalance]);

     const bonusSales = useMemo(() => {
        return sales.filter(s => s.isBonus === true);
    }, [sales]);

    const filteredBonusSales = useMemo(() => {
        let filtered = bonusSales;

        if (bonusFilters.startDate) {
            const start = new Date(bonusFilters.startDate);
            start.setHours(0, 0, 0, 0);
            filtered = filtered.filter(s => new Date(s.saleDate) >= start);
        }
        if (bonusFilters.endDate) {
            const end = new Date(bonusFilters.endDate);
            end.setHours(23, 59, 59, 999);
            filtered = filtered.filter(s => new Date(s.saleDate) <= end);
        }
        if (bonusFilters.affiliateId !== 'all') {
            filtered = filtered.filter(s => s.affiliateId === bonusFilters.affiliateId);
        }
        if (bonusFilters.notesQuery.trim()) {
            const query = bonusFilters.notesQuery.trim().toLowerCase();
            filtered = filtered.filter(s => s.notes?.toLowerCase().includes(query));
        }

        return filtered.sort((a, b) => new Date(b.saleDate).getTime() - new Date(a.saleDate).getTime());
    }, [bonusSales, bonusFilters]);

    const handleBonusFilterChange = (filterName: keyof typeof bonusFilters, value: string) => {
        setBonusFilters(prev => ({ ...prev, [filterName]: value }));
    };
    ...
    const handleBonusSubmit = async (e: React.FormEvent) => {
        ...
        if (!bonusRecipientId) {
            setBonusError('Seleziona un utente.');
            return;
        }
        if (isNaN(amount) || amount <= 0) {
            setBonusError('Inserisci un importo valido.');
            return;
        }

        const result = await onAddBonus(bonusRecipientId, amount, bonusNotes);
        if (result.success) {
            setBonusSuccess('Bonus aggiunto con successo!');
            setBonusRecipientId('');
            setBonusAmount('');
            setBonusNotes('');
        } else {
            setBonusError(result.error || 'Si è verificato un errore.');
        }
    };
    ...
    const handleTransferSubmit = async (e: React.FormEvent) => {
        ...
        const result = await onTransferFunds(user.id, verifiedRecipient.id, amount, transferNotes);
        if (result.success) {
    ...
    const navTabs = [
        ...
        { key: 'add_bonus', label: 'Aggiungi Bonus', roles: [UserRole.ADMIN, UserRole.MANAGER] },
        { key: 'bonus_history', label: 'Cronologia Bonus', roles: [UserRole.ADMIN, UserRole.MANAGER] },
        ...
        <div className="p-8">
            ...
            <div className="bg-surface rounded-xl shadow-md">
                ...
                <div className="p-6">
                    ...
                    {activeTab === 'bonus_history' && isManagerial && (
                        <div>
                            <h3 className="text-xl font-bold text-on-surface mb-4">Cronologia Bonus Assegnati</h3>
                            
                            <div className="mb-6 p-4 bg-gray-50 border rounded-lg grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
                                <div>
                                    <label htmlFor="bonus-start-date" className="block text-xs font-medium text-gray-500 mb-1">Data Inizio</label>
                                    <input type="date" id="bonus-start-date" value={bonusFilters.startDate} onChange={e => handleBonusFilterChange('startDate', e.target.value)} className="w-full text-sm border-gray-300 rounded-md shadow-sm"/>
                                </div>
                                <div>
                                    <label htmlFor="bonus-end-date" className="block text-xs font-medium text-gray-500 mb-1">Data Fine</label>
                                    <input type="date" id="bonus-end-date" value={bonusFilters.endDate} onChange={e => handleBonusFilterChange('endDate', e.target.value)} className="w-full text-sm border-gray-300 rounded-md shadow-sm"/>
                                </div>
                                <div>
                                    <label htmlFor="bonus-affiliate" className="block text-xs font-medium text-gray-500 mb-1">Destinatario</label>
                                    <select id="bonus-affiliate" value={bonusFilters.affiliateId} onChange={e => handleBonusFilterChange('affiliateId', e.target.value)} className="w-full text-sm border-gray-300 rounded-md shadow-sm">
                                        <option value="all">Tutti gli Utenti</option>
                                        {bonusWorthyUsers.map(u => <option key={u.id} value={u.id}>{u.name}</option>)}
                                    </select>
                                </div>
                                 <div>
                                    <label htmlFor="bonus-notes" className="block text-xs font-medium text-gray-500 mb-1">Cerca nelle Note</label>
                                    <input type="text" id="bonus-notes" value={bonusFilters.notesQuery} onChange={e => handleBonusFilterChange('notesQuery', e.target.value)} placeholder="Es. prodotto, campagna..." className="w-full text-sm border-gray-300 rounded-md shadow-sm"/>
                                </div>
                            </div>
                               ...
                               <table className="min-w-full divide-y divide-gray-200">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            ...
                                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Importo</th>
                                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Aggiunto Da</th>
                                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Note</th>
                                            <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">ID Bonus</th>
                                        </tr>
                                    </thead>
                                    <tbody className="bg-white divide-y divide-gray-200">
                                        {filteredBonusSales.map(sale => {
                                            const addedByMatch = sale.notes?.match(/Bonus aggiunto da (.*?)\./);
                                            const addedBy = addedByMatch ? addedByMatch[1] : 'N/A';
                                            const noteText = sale.notes?.replace(/Bonus aggiunto da .*?\./, '').trim();
                                            return(
                                                <tr key={sale.id}>
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{new Date(sale.saleDate).toLocaleDateString('it-IT')}</td>
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{sale.affiliateName}</td>
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-green-600">€{sale.commissionAmount.toFixed(2)}</td>
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{addedBy}</td>
                                                    <td className="px-6 py-4 text-sm text-gray-500 max-w-xs truncate" title={noteText}>{noteText}</td>
                                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-mono">{sale.id}</td>
                                                </tr>
                                            )
                                        })}
                                    </tbody>
                                </table>
                                {filteredBonusSales.length === 0 && <p className="text-center text-gray-500 py-8">Nessun bonus trovato con i filtri attuali.</p>}
                            </div>
                        </div>
                    )}
                    {activeTab === 'add_bonus' && (user.role === UserRole.ADMIN || user.role === UserRole.MANAGER) && (
                        (() => {
                            const isManagerAddingBonus = user.role === UserRole.MANAGER;
                            const usersForDropdown = isManagerAddingBonus 
                                ? bonusWorthyUsers.filter(u => (u.role === UserRole.AFFILIATE || u.role === UserRole.CUSTOMER_CARE) && u.id !== user.id)
                                : bonusWorthyUsers.filter(u => u.id !== user.id);
                    
                            return (
                                <form onSubmit={handleBonusSubmit} className="max-w-lg mx-auto space-y-6">
                                    <h3 className="text-xl font-bold text-on-surface mb-4 text-center">
                                        Aggiungi Bonus Manuale
                                    </h3>
                                    {isManagerAddingBonus && (
                                         <p className="text-center text-lg">Tuo saldo disponibile: <strong className="text-green-600">€{((fullUserObject as Manager).currentBalance || 0).toFixed(2)}</strong></p>
                                    )}
                                    <div>
                                        <label htmlFor="bonusUser" className="block text-sm font-medium text-gray-700">Utente</label>
                                        <select id="bonusUser" value={bonusRecipientId} onChange={e => setBonusRecipientId(e.target.value)} required className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm">
                                    ...
                                    <div>
                                        <label htmlFor="bonusAmount" className="block text-sm font-medium text-gray-700">Importo Bonus (€)</label>
                                        <input type="number" id="bonusAmount" value={bonusAmount} onChange={e => setBonusAmount(e.target.value)} required min="0.01" step="0.01" className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm" />
                                    </div>
                                    <div>
                                        <label htmlFor="bonusNotes" className="block text-sm font-medium text-gray-700">Note (Opzionale)</label>
                                        <textarea id="bonusNotes" value={bonusNotes} onChange={e => setBonusNotes(e.target.value)} rows={3} placeholder="Es: Bonus performance mese di Giugno" className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm" />
                                    </div>
                                    {bonusError && <p className="text-red-500 text-sm text-center">{bonusError}</p>}
                                    {bonusSuccess && <p className="text-green-600 text-sm text-center">{bonusSuccess}</p>}
                    ...
                    {activeTab === 'payout' && (isAffiliate || user.role === UserRole.MANAGER || isCustomerCare) && (
                        <form onSubmit={handlePayoutSubmit} className="max-w-lg mx-auto space-y-6">
                            ...
                            {pendingPayoutsAmount > 0 && (
                                <p className="text-center text-sm text-yellow-600 -mt-4 mb-4">
                                    (di cui €{pendingPayoutsAmount.toFixed(2)} in attesa di approvazione)
                                </p>
                            )}
                            <div>
                                <label htmlFor="payoutAmount" className="block text-sm font-medium text-gray-700">Importo da Prelevare (€)</label>
                                <input type="number" id="payoutAmount" value={payoutAmount} onChange={(e) => setPayoutAmount(e.target.value)} required min="1" step="0.01" max={availableToRequest} className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm" />
                            </div>
                            <div>
                    ...
                    {activeTab === 'transfer' && (isAffiliate || user.role === UserRole.MANAGER) && (
                        <form onSubmit={handleTransferSubmit} className="max-w-lg mx-auto space-y-6">
                            ...
                            <div>
                                <label htmlFor="transferNotes" className="block text-sm font-medium text-gray-700">Note (Opzionale)</label>
                                <input type="text" id="transferNotes" value={transferNotes} onChange={(e) => setTransferNotes(e.target.value)} className="mt-1 block w-full px-3 py-2 bg-white border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary focus:border-primary sm:text-sm" />
                            </div>
