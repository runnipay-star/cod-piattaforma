      ...
      {filteredProducts.length > 0 ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
          {filteredProducts.map((product) => {
            ...
            let commissionDisplay: { label: string; value: number | undefined } | null = null;
            switch (userRole) {
              case UserRole.AFFILIATE:
                commissionDisplay = { label: 'Tua Commissione', value: product.commissionValue };
                break;
              case UserRole.LOGISTICS:
                commissionDisplay = { label: 'Tua Commissione (Logistica)', value: product.fulfillmentCost };
                break;
              case UserRole.CUSTOMER_CARE:
                commissionDisplay = { label: 'Tua Commissione (C. Care)', value: product.customerCareCommission };
                break;
              default: // Admin, Manager
                commissionDisplay = { label: 'Commissione Affiliato', value: product.commissionValue };
                break;
            }
            ...
            return (
              <div key={product.id} className="bg-surface rounded-xl shadow-md overflow-hidden flex flex-col group transition-all duration-300 hover:shadow-2xl hover:-translate-y-1">
                <div className="relative">
                  <img className="aspect-square w-full object-cover transition-transform duration-300 group-hover:scale-105" src={product.imageUrl} alt={product.name} />
